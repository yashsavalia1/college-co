...
export const getServerSideProps: GetServerSideProps<Props> = async ({ req, res }) => {
  ...
  const listings = (await pb.collection('listings').getList<ListingRecord>(1, 50)).items;
        ...
        <section className="p-3">
          ...
          <div className="flex flex-wrap gap-8 md:justify-start justify-center">
            {listings.map((listing) => (
              <HomeCard className="w-72" key={listing.id} listing={listing} thumbUrl={listing.imageUrl}></HomeCard>
            ))}
          </div>
        </section>
      </HomeLayout>
